...
class Board():
    ...
    def is_win(self):
        ...
        for col in range(5):
            for row in range(2):
                if (self.position[row, col] == self.position[row + 1, col] ==
                        self.position[row + 2, col] == self.position[row + 3, col] != self.empty_square):
                    return True

        # check for horizontal win
        for row in range(5):
            for col in range(2):
                if (self.position[row, col] == self.position[row, col + 1] ==
                        self.position[row, col + 2] == self.position[row, col + 3] != self.empty_square):
                    return True

        # check for diagonal win (top-left to bottom-right)
        for row in range(2):
            for col in range(2):
                if (self.position[row, col] == self.position[row + 1, col + 1] ==
                        self.position[row + 2, col + 2] == self.position[row + 3, col + 3] != self.empty_square):
                    return True
    ...
    def game_loop(self):
        ...
        while True:
            ...
            #try:                # parse user input (move format [col, row]: 1,2)
